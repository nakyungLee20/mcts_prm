...
def main():
    ...
    # 방법 1: JSONL 형식으로 한 줄씩 쓰기 (추천)
    output_file = "/home/leena/ccc_eval/mcts_prm/samples/math_2000.jsonl"
    with open(output_file, "w", encoding="utf-8") as f:
        for entry in mcrs.math_reward_dataset_streaming(split="train", start=1000, take=1000):
            f.write(json.dumps(entry, ensure_ascii=False) + "\n")
            f.flush()  # 즉시 디스크에 쓰기
    
    print(f"Data saved to {output_file}")

    # JSONL을 일반 JSON으로 변환 (필요시)
    jsonl_to_json(output_file, "/home/leena/ccc_eval/mcts_prm/samples/math_2000_converted.json")
